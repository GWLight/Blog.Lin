...
function sendData_tuling() {
	...
	if (getText != null && getText != "") {
		  $.post(url,
		 {
			  key:"2ac756157fe547d6b96510e4be00c8c2",
			  info:getText,
			  userid:adr
			...
			if(result.code == code_1){
				...
				msg_text = "<a href='"+result.url+"'>☜点我查看☞</a>"+result.text;
			}else if(result.code == code_3){
				msg_text = "<img src='"+result.list[0].icon+"'  width='25px' height='25px'/><a href='"+result.list[0].detailurl+"'>☜点我查看☞</a>"+result.list[0].article;
			}else if(result.code == code_4){
				msg_text = "<a href='"+result.list[0].detailurl+"'>☜点我查看☞</a>"+result.list[0].name+"介绍："+result.list[0].info;
